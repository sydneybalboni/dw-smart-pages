...
const Trivia = () => {
  ...
  const handlePlayAgain = () => {
    // Reset the state to restart the trivia
    ...
    <div className="trivia-section">
      ...
      {!showResult ? (
        ...
        <div className="trivia-result">
          <h3 className="commitment-message">Whoa, that's commitment!</h3>
          <div className="result-box">You've completed the trivia!</div>
          <p className="result-message">{getResultMessage()}</p>
          <div className="score-box">Your Score: {score} / {questions.length}</div>
          {/* Play Again Button */}
